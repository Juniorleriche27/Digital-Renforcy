                ...
                <ul className="proof-ticker-track">
                  {loopMails.map((mail, index) => (
                    <li key={`left-loop-a-${mail.from}-${index}`} className="proof-phone-row">
                      <div className="proof-phone-avatar">F</div>
                      ...
                      <span className="text-[11px] text-slate-400">{mail.time}</span>
                    </li>
                  ))}
                  {loopMails.map((mail, index) => (
            ...
            <div className="dark-card overflow-hidden rounded-2xl">
              ...
              <ul className="divide-y divide-[#24486f]">
                {proofMails.map((mail, index) => (
                  <li
                    key={`${mail.from}-${index}`}
                    className="proof-mail-row flex items-center gap-3 px-4 py-3"
                    style={{ animationDelay: `${index * 0.24}s` }}
                  >
                    <div className="grid h-10 w-10 place-items-center rounded-full bg-[#2c6df2] text-sm font-bold text-white">
                      F
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-semibold text-slate-100">{mail.from}</p>
