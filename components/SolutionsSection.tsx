      ...
      <div className="container-shell">
        <div className="mx-auto max-w-5xl text-center">
          ...
          <p className="mt-4 text-base text-slate-300 md:text-xl">
            Trois expertises complementaires : developpement web intelligent, solutions numeriques
            automatisees et plateformes d&apos;entrainement en ligne.
          </p>
        </div>
        ...
        <div className="mt-12 grid gap-8 md:grid-cols-2">
          {solutions.map((solution, index) => (
            <article
              key={solution.title}
              className={`dark-card solution-card rounded-[28px] border-2 p-8 ${solution.borderClass}`}
              style={{ "--solution-delay": `${index * 120}ms` } as React.CSSProperties}
            >
              <div className="mb-6 flex items-start justify-between">
                <span className="solution-card-icon grid h-14 w-14 place-items-center rounded-2xl border border-[#376597] bg-[#102c52] text-3xl">
                  {solution.icon === "globe" ? "🌐" : "🎯"}
                </span>
                <span className="solution-card-echo text-2xl">{index === 0 ? "🌐" : "🎯"}</span>
              </div>
